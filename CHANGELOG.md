...
- **feat(llm): server-side proxy for OpenAI / Anthropic / Gemini** — New `POST /api/v1/llm/complete` (Editor or Admin) takes `{ provider, model, prompt, max_tokens?, temperature? }` and forwards to the corresponding provider server-side. Provider API keys never leave the server. Keys are resolved from env vars (`FERRESDB_OPENAI_API_KEY`, `FERRESDB_ANTHROPIC_API_KEY`, `FERRESDB_GEMINI_API_KEY`) with precedence over the new SQLite store `llm_credentials(provider PK, api_key, updated_at)`. Admin CRUD via `GET/PUT/DELETE /api/v1/admin/llm-credentials[/{provider}]` — GET never returns the key value, only `configured`/`source`. Audit entry `action="llm_complete"` records `model`, `prompt_bytes`, `key_source` — never the prompt content. Prometheus metric `ferresdb_llm_proxy_requests_total{provider,status}` (status: ok, upstream_error, auth_error, timeout, error). Dashboard: removed `callOpenAI`/`callAnthropic`/`callGemini` fetch helpers from `QueryTester.tsx`; the page now calls `llmApi.complete(...)`. The "API Key" input in Query Tester is now labeled "Embedding API Key" with a note that LLM keys are server-side. New "LLM Credentials" Admin section in `Settings.tsx`. Integration test `crates/server/tests/llm_proxy_test.rs` (11 cases) drives `wiremock` via `FERRESDB_LLM_PROXY_BASE_URL` env var. Documentation: `docs/api.md` section "LLM Proxy"; README "LLM Proxy keys".
