...
async fn openai_embed_batch(
    ...
    let mut by_index: Vec<(u64, Vec<f32>)> = data_arr
        ...
        .filter_map(|o| {
            ...
            let v: Vec<f32> = emb.iter().filter_map(|e| e.as_f64().map(|f| f as f32)).collect();
            Some((idx, v))
        })
        .collect();
...
/// Um único texto → vetor (chamada à API OpenAI).
...
fn make_http_client(api_key: Option<&str>) -> Result<reqwest::Client, Box<dyn std::error::Error + Send + Sync>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let api_key = cli.api_key.or_else(|| std::env::var("FERRESDB_API_KEY").ok());
    ...
    match &cli.command {
        Commands::Ingest {
            ...
            let openai_key = openai_api_key.clone().or_else(|| std::env::var("OPENAI_API_KEY").ok());
            let dim = dim.or(openai_key.as_ref().map(|_| embedding_dimension_for_model(embedding_model)));
...
async fn run_ingest(
    ...
    println!("  vectors: {}, dim: {}, concurrency: {}", total_vectors, dim, concurrency);
    ...
    match client.create_collection(collection, dim, "Cosine", false).await {
        Ok(_) => println!("  Collection '{}' created (dim={}, distance=Cosine).", collection, dim),
        Err(e) => {
            let msg = e.to_string();
        ...
        for w in 0..concurrency {
            ...
            let h = tokio::spawn(async move {
                for b in batch_start..batch_end {
                    ...
                    let texts: Vec<String> = (0..count).map(|i| format!("Document {} content for benchmark.", base + i)).collect();
                ...
                for b in batch_start..batch_end {
                    ...
                    let points: Vec<PointInput> = (0..count)
                        .map(|i| {
                            ...
                            PointInput { id, vector, metadata: None }
                        })
                        .collect();
    ...
    let metrics = vec![
        ...
        ("Throughput (vectors/s)".to_string(), format_num(throughput as u64)),
    ];
...
async fn run_search(
    ...
    println!("  duration: {:?}, concurrency: {}, collection: {}", duration, concurrency, collection);
    if openai_api_key.is_some() {
        println!("  query embeddings: OpenAI ({})", embedding_model);
    } else {
        println!("  query vectors: random");
    }

    let hist = Arc::new(tokio::sync::RwLock::new(
        Histogram::<u64>::new_with_bounds(1, 60_000_000_000, 3).unwrap(),
    ));
    let count = Arc::new(AtomicU64::new(0));

    let elapsed = if openai_api_key.is_some() {
        // OpenAI path: one embedding API call per query (API-bound); keep loop-based concurrency.
        let start = Instant::now();
        ...
        pb.set_message("running searches (OpenAI)...");
        ...
        for _ in 0..concurrency {
            ...
            let openai_key = openai_api_key.map(String::from);
            let model = embedding_model.to_string();
            let h = tokio::spawn(async move {
                barrier.wait().await;
                let mut query_idx: u64 = 0;
                while Instant::now() < deadline {
                    let q = format!("Query about topic {} for benchmark search.", query_idx);
                    query_idx += 1;
                    let vector = match openai_embed_one(&q, openai_key.as_deref().unwrap(), &model, &http_client).await {
                        Ok(v) => v,
                        Err(_) => continue,
                    };
                    let t0 = Instant::now();
                    let res = client.search(&coll, &vector, 10, None).await;
                    let elapsed_ns = t0.elapsed().as_nanos() as u64;
                    if res.is_ok() {
                        count.fetch_add(1, Ordering::Relaxed);
                        let mut h = hist.write().await;
                        let _ = h.record(elapsed_ns);
                    }
                }
            });
            handles.push(h);
        ...
        // Random-vector path: single shared client, pre-generated vectors, stream + buffer_unordered.
        let total_requests = (duration.as_secs() as usize)
            .saturating_mul(15_000)
            .max(concurrency * 1000);
        println!("  pre-generating {} query vectors (excluded from timing)...", format_num(total_requests as u64));
        let mut rng = rand::rngs::StdRng::from_entropy();
        let vectors: Vec<Vec<f32>> = (0..total_requests).map(|_| random_vector(dim, &mut rng)).collect();
        println!("  starting timed run (concurrency={})...", concurrency);

        let client = Arc::new(client);
        let coll = collection.to_string();
        let bodies = stream::iter(0..vectors.len())
            .map(|i| {
                let client = client.clone();
                let coll = coll.clone();
        ...
        let start_timed = Instant::now();
        let _ = tokio::time::timeout(duration, bodies.collect::<Vec<_>>()).await;
        start_timed.elapsed()
    };
    ...
    let (lat_avg, lat_p95, lat_p99) = {
        let h = hist.read().await;
        let avg = if total > 0 { h.mean() } else { 0.0 };
        let p95 = if total > 0 {
    ...
    println!("\nSearch complete.");
    println!("  Total requests: {}", format_num(total));
    println!("  Duration: {}", format_duration(elapsed));
...
async fn run_chaos(
    ...
    println!("  duration: {:?}, writers: {}, readers: {}", duration, writers, readers);

    let base_name = "bench_chaos";
    match client.create_collection(base_name, dim, "Cosine", false).await {
